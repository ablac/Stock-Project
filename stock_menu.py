...
from Imports.account_class import Traditional, Robo
...
def delete_stock(stock_list):
    ...
    print("]")
    symbol = input("Which stock do youi want to delete?: ").upper()
    found = False
    i = 0
    for stock in stock_list:
...
def list_stocks(stock_list):
    ...
    for stock in stock_list:
        print(stock.symbol, " " * (14 - len(stock.symbol)), stock.name,
              " " * (14 - len(stock.name)), stock.shares)
    print()
    _ = input("Press enter to continue")
...
def obj_encoder(obj):
    ...
    for o in obj.DataList:
        d = data_encoder(o)
        dlist.append(d)
    stock_dict = dict(symbol=obj.symbol,
                      name=obj.name,
                      shares=obj.shares,
                      DataList=dlist)
    return stock_dict
...
def file_processing(stock_list):
    ...
    while choise != "E":
        choise = input(
            "Do you want to save data (S), Load data (L) Inport Data (D) or Exit (E) "
        ).upper()
        if choise == "S":
            json_list = [obj_encoder(stock) for stock in stock_list]
            json_dict["Stock"] = json_list
            try:
                with open("SCV/Saved/stock_data.json", 'w') as f:
            ...
            try:
                with open("CSV/Saved/stock_data.json", "r") as f:
                    str_file = f.read()
...
def import_stock_csv(stock_list, symbol, filename):
    for stock in stock_list:
        if stock.symbol == symbol:
            with open('CSV/' + filename, newline='') as stockdata:
