...
class StockApp:
    def __init__(self):
        ...
        self.webmenu.add_command(label="Import CSV from Yahoo! Finance...",
        ...
        self.chartmenu.add_command(label="Display Stock Chart",
        ...
        # Set Up Main Tab
        self.addStockGroup = LabelFrame(self.mainFrame,
                                        text="Add Stock",
                                        padx=5,
                                        pady=5)
    ...
    def display_stock_data(self):
        ...
        for stock in self.stock_list:
            if stock.symbol == symbol:
                self.headingLabel['text'] = stock.name + " - " + str(
                    stock.shares) + " Shares"
                ...
                for daily_data in stock.DataList:
                    row = daily_data.date + "   " + '${:0,.2f}'.format(
                        daily_data.close) + "   " + str(
                            daily_data.volume) + "\n"
                    self.dailyDataList.insert(END, row)
                    currentDate = datetime.strptime(daily_data.date,
                                                    "%Y-%m-%d")
                #display report
                count = 0
                price_total = 0.00
                ...
                if count > 0:
                    ...
                    self.stockReport.insert(
                        END,
                        "Low Price: " + "${:,.2f}".format(lowPrice) + "\n")
                    self.stockReport.insert(
                        END,
                        "High Price: " + "${:,.2f}".format(highPrice) + "\n")
                    self.stockReport.insert(
                        END, "Average Price: " +
                        "${:,.2f}".format(price_total / count) + "\n\n")
                    self.stockReport.insert(
                        END, "Low Volume: " + str(lowVolume) + "\n")
                    self.stockReport.insert(
                        END, "High Volume: " + str(highVolume) + "\n")
                    self.stockReport.insert(
                        END, "Average Volume: " +
    ...
    def add_stock(self):
        new_stock = Stock(self.addSymbolEntry.get(), self.addNameEntry.get(),
            ...
            if stock.symbol == symbol:
                with open(filename, newline='') as stockdata:
                    ...
                    for row in datareader:
                        daily_data = DailyData(str(row[0]), float(row[4]),
                                               float(row[6]))
